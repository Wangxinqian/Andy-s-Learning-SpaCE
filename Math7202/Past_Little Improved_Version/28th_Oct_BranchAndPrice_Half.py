...
def GenColumns():
    #For every starting depot
    for d,i in CanReach:
        Short = [(GRB.INFINITY,None) for t in Trips]
        Short[i] = (BusCost + Depot_TripFuel[d][i]-Cover[i].pi,None)
        for j in CanReach[d,i]:
            #is the column that finishes at j a candidate
            if Short[j][0]+Trip_DepotFuel[j][d]<-0.0001:
                tCol = []
                upto = j
                while upto!=None:
                    tCol.append(upto)
