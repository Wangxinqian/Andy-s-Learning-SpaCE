...
####
##这里又错误 有的时候第一段trip 就是 infeasible了
#####
CanReach = {(d,k):
    [[k,FuelCapacity-Depot_TripFuel[d][k]-TripFuel[k]]]+[r for r in CanFuel_Decomposition([i for i in Trips],k,FuelCapacity-Depot_TripFuel[d][k]-TripFuel[k])]
...
Successors = {(d,(k,t[0])):
    [r for r in CanFuel_Decomposition([i[0] for i in CanReach[d,k][1:]],t[0],t[-1])]
    for (d,k) in CanReach for t in CanReach[d,k][1:]}
