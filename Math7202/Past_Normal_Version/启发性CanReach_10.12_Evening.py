...
def Generate(Tour,d):
    ...
    if (d,k[-1]) in CanReach and len(CanReach[d,k[-1]]) > 1:
        ...
        #Check if can get the depot
        if Tour[-1][-1] >= Trip_DepotFuel[Tour[-1][0]][d]:
            AllTours[d,k] = cost+Trip_DepotFuel[Tour[-1][0]][d]
            if len(Tour)>=3:
                return
        else:
            return False
...
# DepotCapacityConstrain = {d:
#     m.addConstr(quicksum(Z[k] for k in Z if d == k[0])<=DepotCapacity)
#     for d in Depots}
...
#         Short[i] = (Depot_TripFuel[d][i]+TripFuel[i]-Cover[i].pi,None)
#         for j in CanReach[d,i]:
#             if Short[j][0]
#             if Short[j][0]==GRB.INFINITY:
#                 print('Failed at',d,j,i)
#                 xxxxxxxx
#             for t in Successors[d,i,j]:
#                 tCost = Short[j][0]+D[TripLoc[j][1]][t0]
#                 if t Cost<Short[t][0]:
#                     Short[t] = (tCost,j)
