...
CanReach = {(d,k):
    [k]+[t for t in Trips
        if all([
                ...
                any([
                    # do the trip t and can get to the nearest fuel station
                    TripFuel[t] + Trip_TripFuel[k][t] + min(Trip_FuelStationFuel[t][station] for station in Fuels) <= FuelCapacity - Depot_TripFuel[d][k] - TripFuel[k],
                    # OR go to fuel station to add some fuel
                    min([FuelStation_TripFuel[station][t]
                        for station in Fuels 
                        if TripTime[t][0] - Trip_FuelStationTime[k][station] - FuelStation_TripTime[station][t] >= 0 #Time enough
                        #Fuel enough to station
                        and FuelCapacity-Depot_TripFuel[d][k]-TripFuel[k] >= Trip_FuelStationFuel[k][station]], default = -999999) <= FuelCapacity - TripFuel[t] - min(Trip_FuelStationFuel[t][station] for station in Fuels)
                    ])==True
        ])==True
        ]
